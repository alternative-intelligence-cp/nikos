...
public:
  ...
  void taint_set_memory_tainted(VariableRef) {
    // DummyDomain does not track memory cells, so we can't taint memory here.
  }

  bool taint_is_memory_tainted(VariableRef) const {
    // DummyDomain does not track memory cells.
    return false;
  }
