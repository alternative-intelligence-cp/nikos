# NIKOS v1.0.0 — Installation Guide for Ubuntu 22.04 LTS (Jammy Jellyfish)
...
NIKOS is a static analyzer for C/C++ programs built on top of LLVM. It uses abstract interpretation to detect bugs such as buffer overflows, null-pointer dereferences, integer overflows, and more. This guide covers a complete installation from scratch on Ubuntu 22.04 LTS.

Ubuntu 22.04 ships **LLVM 14** in its default repositories. NIKOS v1.0.0 requires **LLVM 20**, so you must add the official LLVM apt repository before installing.
...
| LLVM | 20 | 20 |
| CMake | 3.14+ | 3.22+ |
| GCC/G++ | 10+ | 11 (default on 22.04) |

---

## 3. Install LLVM 20 and Clang 20

Ubuntu 22.04's default repositories do not include LLVM 20. Use the official LLVM installation script from [apt.llvm.org](https://apt.llvm.org).

### 3.1 Download and Run the LLVM Installation Script
...
- Adds the LLVM apt repository for Ubuntu 22.04 (Jammy)
- Imports the LLVM GPG signing key
- Installs `llvm-20`, `clang-20`, `lld-20`, and related packages
...
### 3.3 (Optional) Set LLVM 20 as Default via update-alternatives

If you want `clang` and `clang++` to point to version 20:
...
| Package | Purpose |
|---------|---------|
| `cmake` | Build system generator (3.22 ships with 22.04) |
...
| `libtbb-dev` | Intel Threading Building Blocks (parallel analysis passes) |
...
Always build out-of-source to keep the repository clean:
...
`$(nproc)` automatically uses all available CPU cores. On a machine with 4 cores this runs 4 parallel jobs. Compilation typically takes 5–15 minutes depending on hardware.

> **Tip:** If the build fails intermittently with out-of-memory errors, reduce parallel jobs:
> ```bash
> make -j2
> ```
...
Use `clang-20` with NIKOS's preprocessor to produce the annotated bitcode file. NIKOS typically uses a wrapper called `ikos-pp` for this step:
...
APRON is a C library that provides 13 additional abstract domains (polyhedra, octagons extended, etc.) beyond NIKOS's built-in domains. It is optional but recommended for more precise analysis.
...
| Package | Purpose |
|---------|---------|
| `libmpfr-dev` | GNU MPFR multi-precision floating-point library |
| `libppl-dev` | Parma Polyhedra Library (enables PPL-based domains in APRON) |
...
The LLVM 20 packages are not installed or not on your PATH. Re-run the installation script:

```bash
wget https://apt.llvm.org/llvm.sh && chmod +x llvm.sh && sudo ./llvm.sh 20
```

Then verify: `which llvm-config-20`

### CMake fails: `Could not find LLVM`
...
Check that all apt dependencies were installed correctly:
...
Ensure the bitcode was compiled with debug info (`-g`) and without optimizations (`-O0`). The analyzer requires unoptimized, annotated bitcode.
...
Or, if memory is very constrained:
...
Either use `sudo make install` (if installing to a system path like `/opt/nikos`) or ensure the install prefix is writable by your user:
