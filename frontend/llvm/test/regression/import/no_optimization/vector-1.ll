; ModuleID = 'vector-1.pp.bc'
source_filename = "vector-1.c"
...
@a = common global <4 x float> zeroinitializer, align 16, !dbg !0
; CHECK: define <4 x float>* @a, align 16, init {
; CHECK: #1 !entry !exit {
; CHECK:   store @a, aggregate_zero, align 1
; CHECK: }

@b = common global <4 x float> zeroinitializer, align 16, !dbg !6
; CHECK: }
; CHECK: define <4 x float>* @b, align 16, init {
; CHECK: #1 !entry !exit {
; CHECK:   store @b, aggregate_zero, align 1

@c = common global <4 x float> zeroinitializer, align 16, !dbg !13
; CHECK: }
; CHECK: }
; CHECK: define <4 x float>* @c, align 16, init {
; CHECK: #1 !entry !exit {
...
define i32 @main() #0 !dbg !20 {
  ...
  %2 = load <4 x float>, <4 x float>* @a, align 16, !dbg !24
  %3 = load <4 x float>, <4 x float>* @b, align 16, !dbg !25
  %4 = fadd <4 x float> %2, %3, !dbg !26
  store <4 x float> %4, <4 x float>* @c, align 16, !dbg !27
  ret i32 0, !dbg !28
}
...
; CHECK:   <4 x float> %2 = load @a, align 16
; CHECK:   <4 x float> %3 = load @b, align 16
; CHECK:   <4 x float> %4 = %2 fadd %3
